...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.union.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.union.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.union.lock.RLock()
	defer api.union.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.union.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.union.lock.Lock()
	defer api.union.lock.Unlock()

	api.union.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.union.proposals, address)
}
...
// GetSigner returns the signer for a specific union block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.union.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.union.Author(block.Header())
	}
