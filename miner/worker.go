...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && (w.chainConfig.Clique == nil || w.chainConfig.Clique.Period > 0 || w.chainConfig.Union == nil || w.chainConfig.Union.Period > 0) {
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				} else if w.chainConfig.Union != nil && w.chainConfig.Union.Period == 0 {
					w.commitNewWork(nil, true, time.Now().Unix())
				}
			}
